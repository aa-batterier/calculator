...
{
	public:
		List(const int size = 0,const int capacity = 0,const char *list = nullptr) : _size(size),_capacity(capacity),_list(list) {}
		List(const List& copy) : _size(copy._size),_capacity(copy._capacity);
